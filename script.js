...
window.onload = async () => {
    ...
    // INTEGRAZIONE: Caricamento database profumi brand
...
function resetConfiguratore() {
    ...
    // Rimuove anche le miniature dei profumi brand se presenti
    const brandWrapper = document.getElementById('brand-suggestions-wrapper');
    if (brandWrapper) brandWrapper.remove();
...
function mostraSceltaEssenza() {
    ...
    // INTEGRAZIONE: Aggiorna miniature profumi brand
    aggiornaMiniatureProfumi(sotto);
...
// INTEGRAZIONE: Nuova funzione per le miniature profumi brand correlati
function aggiornaMiniatureProfumi(sottofamigliaScelta) {
    ...
    wrapper.innerHTML = `<p style="font-size: 10px; color: #888; text-transform: uppercase; margin-bottom: 10px;">Profumi di riferimento:</p>`;
    ...
    correlati.forEach(p => {
        ...
        item.innerHTML = `
            <div style="width: 50px; height: 50px; background: #fff; border-radius: 4px; display: flex; align-items: center; justify-content: center; overflow: hidden; border: 1px solid #444; margin-bottom: 5px;">
                <img src="assets/${ean}.png" style="max-width: 90%; max-height: 90%; object-fit: contain;" onerror="this.src='https://via.placeholder.com/50?text=?'">
            </div>
...
// Funzioni colore e etichetta invariate
